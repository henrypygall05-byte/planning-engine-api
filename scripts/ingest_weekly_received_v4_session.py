...
def fetch_get(sess: requests.Session, params: dict) -> Tuple[str, str, int]:
...
def extract_form_payload(html: str) -> dict:
    """
    Find the form that contains the 'week' input and return all hidden fields + fa.
    This makes us robust to servers that add tokens.
    """
    ...
    if form:
        for inp in form.find_all("input"):
            ...
            # include hidden inputs + required 'fa' if present
            if itype in ("hidden",):
                payload[name] = val
...
def post_week(sess: requests.Session, week_val: str) -> Tuple[str, str, int]:
    # 1) GET the form to establish cookies + capture hidden fields
    url0, html0, st0 = fetch_get(sess, {"fa": FA_VALUE})
    payload = extract_form_payload(html0)
    # 2) POST the week query using cookies + hidden fields
    payload["week"] = week_val

    r = sess.post(URL, data=payload, timeout=TIMEOUT, allow_redirects=True)
    return r.url, r.text, r.status_code
...
def main():
    ...
    ap.add_argument("--start-week", type=str, default="", help="DD/MM/YYYY (optional override)")
    args = ap.parse_args()
    ...
    try:
        ...
        for w in weeks:
            ...
            url, html, status = post_week(sess, week_val)

            if status >= 400:
                weeks_failed += 1
                print(f"Week {week_iso} | HTTP {status} | failed")
                continue
