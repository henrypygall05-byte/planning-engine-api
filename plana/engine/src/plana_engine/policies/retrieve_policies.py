...
def _score_text(text: str, q_tokens: List[str], topic: str) -> float:
    # Topic-aware scoring + policy-first bias + noise suppression
    if not text:
        return 0.0
    t = text.lower()

    # Hard noise suppression
    ...
    if topic == "householder":
        # strongly prefer amenity/design content
        if any(k in t for k in ["residential amenity","daylight","sunlight","privacy","overlooking","outlook","spacing","scale","massing","materials"]):
            score *= 1.50

        # penalise heritage-heavy material
        if any(k in t for k in ["heritage","conservation area","listed","battlefield","significance of the heritage"]):
            score *= 0.20

        # penalise other common noise for householders
        if any(k in t for k in ["open space","allotments","parks and recreation","waste","minerals","safeguarded","aerodrome"]):
            score *= 0.25

        # guardrail: if it's a householder query and the chunk has NONE of the amenity/design terms, downweight
        if not any(k in t for k in ["amenity","daylight","sunlight","privacy","overlooking","outlook","scale","massing","materials","design"]):
            score *= 0.35

    elif topic == "heritage":
        if any(k in t for k in ["heritage","conservation area","listed","significance","setting"]):
            score *= 1.35

    elif topic == "highways":
        if any(k in t for k in ["highway","parking","access","junction","traffic","visibility"]):
            score *= 1.35

    elif topic == "flood":
        if any(k in t for k in ["flood","drainage","surface water","suds"]):
            score *= 1.35
...
def retrieve_policies(
    ...
    for r in rows:
        txt = r["text"] or ""
        s = _score_text(txt, q_tokens, topic)
        if s <= 0:
            continue
