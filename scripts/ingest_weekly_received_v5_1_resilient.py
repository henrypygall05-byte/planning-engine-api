...
def fetch_form(sess: requests.Session) -> Tuple[int, str]:
...
def fetch_post(sess: requests.Session, week_val: str) -> requests.Response:
    st, html0 = fetch_form(sess)
    if st != 200:
        # Return a synthetic response-like object by doing a POST without hidden fields;
        # caller will handle status/behaviour.
        return sess.post(URL, data={"fa": FA_VALUE, "week": week_val}, timeout=TIMEOUT, allow_redirects=True)
...
def fetch_week_resilient(sess: requests.Session, week_val: str) -> Tuple[str, str, int, str]:
    ...
    # retries with deterministic backoff and fresh sessions
    for b in BACKOFFS:
        time.sleep(b)
        s2 = get_session()
        try:
            r = fetch_get(s2, week_val)
            if r.status_code == 200:
                return r.url, r.text, r.status_code, f"GET-retry({b})"
            r = fetch_post(s2, week_val)
            if r.status_code == 200:
...
def main():
    ...
    ap.add_argument("--start-week", type=str, default="")
    args = ap.parse_args()
